...
The raw binary (`src-tauri/target/release/vozel`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/Vozel/src-tauri/target/release/bundle/deb/Vozel_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/vozel /usr/bin/
sudo cp -a usr/lib/. /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/Vozel.desktop /usr/share/applications/
```

The runtime libraries live in the app-private `/usr/lib/Vozel/` (on the binary's rpath), so no `ldconfig` step is needed.

After subsequent rebuilds, copy the binary and any refreshed runtime libraries:

```bash
sudo cp src-tauri/target/release/vozel /usr/bin/
sudo mkdir -p /usr/lib/Vozel
sudo cp -a src-tauri/transcribe-libs/. /usr/lib/Vozel/
```
...
remains visibly enabled, leaving Vozel on `Waiting...`.

After installing the final bundle at `/Applications/Vozel.app`, quit Vozel, clear only its
stale Accessibility record, then reopen it:

```bash
osascript -e 'tell application id "io.github.renatoximenes.vozel" to quit' || true
tccutil reset Accessibility io.github.renatoximenes.vozel
open /Applications/Vozel.app
```
...
```bash
codesign -dr - /path/to/previous/Vozel.app 2>&1
codesign -dr - /Applications/Vozel.app 2>&1
```
...
See the upstream issue history for the related onboarding and stale-permission
report; report Vozel-specific build issues in this repository.
...
```
Bundling Vozel_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Vozel.AppDir --plugin gtk --output appimage
```
...
# Rust projects' build output, not just Vozel):
...
If the build compiles all the way to `Built application at: ...\vozel.exe` and
then fails with:

```
Signing C:\...\vozel.exe with a custom signing command
