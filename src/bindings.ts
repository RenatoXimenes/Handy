...
async saveTranscriptionEndpoint(input: TranscriptionEndpointInput, secret: string | null) : Promise<Result<TranscriptionEndpointView, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("save_transcription_endpoint", { input, secret }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
