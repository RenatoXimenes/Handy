//! Portable mode support for Vozel, including legacy marker compatibility.
//!
...
pub fn init() {
    PORTABLE_DATA_DIR.get_or_init(|| {
        ...
        let is_portable = if is_valid_portable_marker(&marker_path) {
            ...
            let _ = std::fs::write(&marker_path, "Vozel Portable Mode");
            true
        } else {
            false
        };
...
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| {
            let marker = s.trim();
            marker.starts_with("Vozel Portable Mode") || marker.starts_with("Handy Portable Mode")
        })
        .unwrap_or(false)
}
